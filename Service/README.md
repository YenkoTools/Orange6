...
```
GET http://localhost:7071/api/oauth2-redirect.html
```
...
```
GET http://localhost:7071/api/openapi/{version}.{extension}
```
...
```
GET http://localhost:7071/api/openapi/v2.json
GET http://localhost:7071/api/openapi/v3.yaml
```

**How to use:**
- Open the URL in a browser or HTTP client to inspect the spec.
- Import `http://localhost:7071/api/openapi/v3.json` into Postman via **File → Import → Link** to auto-generate a collection.
...
```
GET http://localhost:7071/api/swagger.{extension}
```
...
```
GET http://localhost:7071/api/swagger.ui      ← Interactive Swagger UI
GET http://localhost:7071/api/swagger.json    ← Raw Swagger 2.0 spec as JSON
GET http://localhost:7071/api/swagger.yaml    ← Raw Swagger 2.0 spec as YAML
```

**How to use:**
1. Open `http://localhost:7071/api/swagger.ui` in a browser.
